...
// A reference field holds linked entries; plain value arrays (e.g. strings) have no `sys`.
const isEntryArray = value =>
  Array.isArray(value) && value.length > 0 && value.every(item => item?.sys);
...
export async function getPageReferenceEntries({ page }) {
  ...
  return Promise.all(
    blocks.map(({ fields }) => {
      const linkedEntries = Object.values(fields).find(isEntryArray);
